var Simulator = (function () {
    ...
    function Simulator (wgl, shaderSources, resolutionWidth, resolutionHeight) {
        ...
        this.diffuseProgram = wgl.createProgram(
            shaderSources['shaders/fullscreen.vert'], shaderSources['shaders/diffuse.frag']);
    ...
    Simulator.prototype.splat = function (brush, zThreshold, paintingRectangle, splatColor, splatRadius, velocityScale, isEraser, isDry, isUnified) {
        ...
        */
        ...
        var splatPaintDrawState = wgl.createDrawState()
            ...
            .blendFuncSeparate(
                isEraser ? wgl.ZERO : wgl.SRC_ALPHA,
                isEraser ? wgl.ONE_MINUS_SRC_ALPHA : wgl.ONE_MINUS_SRC_ALPHA,
                isEraser ? wgl.ZERO : (isDry ? wgl.SRC_ALPHA : wgl.ONE),
                isEraser ? wgl.ONE_MINUS_SRC_ALPHA : (isDry ? wgl.ONE_MINUS_SRC_ALPHA : wgl.ONE)
            )
        ...
        var splatVelocityDrawState = wgl.createDrawState()
            ...
            .enable(wgl.BLEND)
            .blendEquation(wgl.FUNC_ADD)
            .blendFuncSeparate(wgl.ONE, wgl.ONE, wgl.ONE, wgl.ONE_MINUS_SRC_ALPHA)

            .vertexAttribPointer(brush.splatCoordinatesBuffer, this.velocitySplatProgram.getAttribLocation('a_splatCoordinates'), 4, wgl.FLOAT, wgl.FALSE, 0, 0)
            .bindIndexBuffer(brush.splatIndexBuffer)
    ...
    Simulator.prototype.simulate = function (tool) {
        var wgl = this.wgl;
        ...
        if (tool === 'smudge' || tool === 'blur') {
            var mixAmount = (tool === 'blur') ? 0.65 : 0.25; // stronger blur/diffusion for blur tool
            var diffuseDrawState = wgl.createDrawState()
                .bindFramebuffer(this.simulationFramebuffer)
                .viewport(simulationArea.left, simulationArea.bottom, simulationArea.width, simulationArea.height)
                ...
                .useProgram(this.diffuseProgram)
                .uniform1f('u_mixAmount', mixAmount) // A nice, smooth and chill blend amount
                .uniform2f('u_resolution', this.resolutionWidth, this.resolutionHeight)
                .uniform1i('u_bilateral', (tool === 'blur') ? 0 : 1)
                .uniformTexture('u_paintTexture', 0, wgl.TEXTURE_2D, this.paintTexture)
                .uniformTexture('u_velocityTexture', 1, wgl.TEXTURE_2D, this.velocityTexture)

                .vertexAttribPointer(this.quadVertexBuffer, this.diffuseProgram.getAttribLocation('a_position'), 2, wgl.FLOAT, false, 0, 0);

            wgl.framebufferTexture2D(this.simulationFramebuffer, wgl.FRAMEBUFFER, wgl.COLOR_ATTACHMENT0, wgl.TEXTURE_2D, this.paintTextureTemp, 0);
            wgl.drawArrays(diffuseDrawState, wgl.TRIANGLE_STRIP, 0, 4);
            Utilities.swap(this, 'paintTexture', 'paintTextureTemp');
        }

        //advect velocity
        // For ink, we want it to dry out relatively fast, so we apply a high dissipation rate (low fluidity multiplier)
        // For dry media, it dissipates instantly (0.0) resulting in static predictable strokes
        var velocityDissipation;
        if (tool === 'ink' || tool === true) {
            velocityDissipation = this.fluidity * 0.42;
        } else if (tool === 'dry') {
            velocityDissipation = 0.0;
        } else {
            velocityDissipation = this.fluidity;
        }
        advect(this.velocityTexture, this.velocityTexture, this.velocityTextureTemp, DELTA_TIME, velocityDissipation);
        Utilities.swap(this, 'velocityTexture', 'velocityTextureTemp');
