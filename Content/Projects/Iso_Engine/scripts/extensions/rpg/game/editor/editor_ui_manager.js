...
class EditorUIManager {
    ...
    promptForMapName(currentName = '') {
        return new Promise((resolve) => {
            const mapName = prompt("Enter a name for this map:", currentName || `MyMap_${Date.now()}`);
            resolve(mapName); 
        });
    }
        ...
        if (maps.length === 0) {
            ...
            maps.forEach(mapInfo => {
                ...
                deleteButton.onclick = (e) => {
                    e.stopPropagation();
                    if (confirm(`Are you sure you want to delete the map "${mapInfo.name}"? This cannot be undone.`)) {
