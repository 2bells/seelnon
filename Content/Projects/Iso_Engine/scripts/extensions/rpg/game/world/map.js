    ...
    serialize() {
        ...
        for (const key in this.objectLayersData) {
            serializableObjectLayersData[key] = this.objectLayersData[key].map(objData => {
                const { engine, sprite, ...serializableData } = objData; // Don't serialize engine or live sprite
        ...
        return {
            ...
            customSpritesheets: JSON.parse(JSON.stringify(this.customSpritesheets)),
            tileDefinitions: JSON.parse(JSON.stringify(this.tileDefinitions)), 
            nextTileId: this.nextTileId,
            objectLayersData: serializableObjectLayersData, 
            collisionLayerData: JSON.parse(JSON.stringify(this.collisionLayerData.map(shape => ({
                id: shape.id, 
                vertices: shape.vertices 
            })))),
            occlusionLayerData: JSON.parse(JSON.stringify(this.occlusionLayerData.map(shape => ({
                id: shape.id,
                vertices: shape.vertices
            })))),
            spawnPointsData: JSON.parse(JSON.stringify(this.spawnPointsData.map(point => ({
                id: point.id,
                x: point.x,
                y: point.y,
                type: point.type,
                targetMap: point.targetMap,
                npcData: point.npcData || null,
                enemyId: point.enemyId || null
            })))),
            lightingData: JSON.parse(JSON.stringify(this.lightingData))
        };
    }
