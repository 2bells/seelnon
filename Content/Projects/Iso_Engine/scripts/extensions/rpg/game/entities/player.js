...
class Player {
    ...
    takeDamage(amount, attacker) {
        const damageTaken = Math.max(1, amount - this.stats.def);
    ...
    findNewTarget() {
        ...
        for (const obj of this.engine.gameObjects) {
            if (obj instanceof Enemy && obj.stats.hp > 0) {
    ...
    attack(target) {
        console.log(`Player attacks ${target.name}!`);
    ...
    render(ctx, viewOriginX, viewOriginY) {
        ...
        const drawY = this.currentPixelY - viewOriginY; // This is the circle center on canvas
