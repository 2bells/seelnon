    ...
    editor.innerHTML = `
        ...
        <div id="canvas-settings-tab-content" class="brush-editor-tab-content">
            ...
            <div class="canvas-setting">
                ...
                <input type="color" id="canvasBgColor" value="${state.canvasSettings.backgroundColor}">
            </div>
...
export function init(container) {
    ...
    // Canvas Settings elements (now in their own tab)
    ...
    function updateToolDropdowns() {
        ...
        for (const baseType in brushContainers) {
            ...
            if (dropdown) {
                // Clear existing custom presets (keep hardcoded default ones, they have actual IDs)
                Array.from(dropdown.children).forEach(child => {
                    const presetId = child.dataset.presetId;
                    // Only remove if it's a user-defined preset or a dynamic entry
                    // Check if it starts with 'user-custom-' or if it's not one of the explicitly hardcoded ones
                    if (presetId && presetId.startsWith('user-custom-')) {
                        child.remove();
                    }
                });
        ...
        // Ensure the current active main button is marked active
