export class UpgradeManager {
  constructor(game) {
    this.game = game;
    this.upgradeTimer = 0;
    this.currentUpgradeCycle = 1;
    this.maxTier = this.game.mapData?.upgrades?.maxTier || 4;
    this.upgradeInterval = this.game.mapData?.upgrades?.upgradeInterval || 800; // 30 seconds at 60 FPS
  ...
  update() {
    ...
    if (this.upgradeTimer >= this.upgradeInterval) {
      this.upgradeTimer = 0;
      this.performUnitUpgrades();
    }
  }

  performUnitUpgrades() {
    // Track if any upgrades were made this cycle
    let upgradesMade = false;

    this.game.units.forEach(unit => {
      // Only upgrade if unit is below max tier
      if (unit.tier < this.maxTier) {
        unit.tier++;
    ...
    // If upgrades were made, increment upgrade cycle
    if (upgradesMade) {
      this.currentUpgradeCycle++;

      // Announce upgrade
      if (this.game.aiController) {
        this.game.aiController.queueMessage(`Units upgraded to next tier!`);
      }
    }
  }
